...
class InventoryScreen(BaseScreen):
    ...
    def _render_tree(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            # Use unique_id as IID for reliable tracking if possible, else str(idx)
            # If using uid as iid, ensure uniqueness. Our registry ensures it for active items.
            # But duplicate items in different files might share ID? No, logic prevents.
            # Let's use uid as IID.
            self.tree.insert('', tk.END, values=vals, iid=uid)
...
class AnalyticsScreen(BaseScreen):
    def __init__(self, parent, app_context):
        ...
        self.txt_report = tk.Text(self, font=('Consolas', 11), state='disabled', padx=10, pady=10)
        self.txt_report.pack(fill=tk.BOTH, expand=True)
        
        ttk.Button(self, text="Refresh Report", command=self.refresh).pack(pady=10)
...
STOCK SUMMARY
-------------
Total Stock IN (Available + Return): {total_in}
Total Stock OUT (Sold): {total_out}
-------------------------------------
Total Items (All Time): {stats['total_items']}
Total Value (Estimated): ₹{stats['total_value']:,.2f}
